...
project_path = "C:\Apps\The-Prophecy-of-Hank\HandyHank\Assets\HandyHank\Scripts" #input("Enter the project path: ")
...
.replace(" *-- IDisposable", "").replace(" *-- SerializedMonoBehaviour", "")\
...
def generate_mermaid_diagram_chunks(cs_files, project_path):
    ...
    #for cs_file in cs_files:
    #    class_name = get_class_name(cs_file)
    #    for relationship in class_info_dict[cs_file].relationships:
...
mermaid_diagram = unity_format(generate_mermaid_diagram_chunks(cs_files, project_path))
